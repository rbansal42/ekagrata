...
export default function ArtisansPage() {
  ...
  // Get unique specializations from artisans
  const specializations = Array.from(
    new Set(DUMMY_ARTISANS.map(artisan => artisan.attributes.specialization))
  );
  ...
  useEffect(() => {
    const fetchArtisans = async () => {
      ...
      try {
        ...
        setArtisans(response?.data || DUMMY_ARTISANS);
      } catch (error) {
  ...
  const filteredArtisans = artisans.filter(artisan => {
    const matchesSearch = artisan.attributes.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
